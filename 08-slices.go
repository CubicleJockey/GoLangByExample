...
Slices can also be copy’d. Here we create an empty slice cof the same length as s and copy into c from s.
...
func main(){
	...
	s := make([]string, 3)
	fmt.Println("Initialize: ", s, " of length: ", len(s))
	...
	s = append(s, "d")
	s = append(s, "e", "f")
	fmt.Println("Appended Array ", s)
	
	copyOfs := make([]string, len(s))
	copy(copyOfs, s)
	
	fmt.Println("Copy of s: ", copyOfs)
